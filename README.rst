...
By default, the `scipy-wheels` repository is usually set up to build
...
`.travis.yml` in the `scipy-wheels` repository.  You should see something
like::

    - BUILD_COMMIT='latest-tag'

If this is commented out, then the repository is set up to build the current
commit in the `scipy` submodule of the repository.  If it is set to
...
be something like::
    ...
    wheel-uploader -v -w ~/wheelhouse scipy 0.17.0
...
* `0.17.0` is the version to download / upload.

So, in this case, `wheel-uploader` will download all wheels starting with
`scipy-0.17.0-` from http://wheels.scipy.org to `~/wheelhouse`, then upload
them to pypi.
